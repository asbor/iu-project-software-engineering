...
class Category(Base):
    ...
    publications = relationship("Publication", secondary="category_publication")
...
class Publication(Base):
    ...
    authors = relationship("Author", secondary="publication_author", back_populates="publications")
    categories = relationship("Category", secondary="publication_category", back_populates="publications")
...
class Card(Base):
    ...
    magstripe = Column(String(20), CheckConstraint('LENGTH(magstripe) >= 20 AND LENGTH(magstripe) <= 20'), nullable=False)
    status = Column(Enum('active', 'expired', 'inactive', name='card_status'), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class Instance(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=True, nullable=False)
    type = Column(Enum('physical', 'ebook', 'audiobook', 'publisher', name='instance_type_enum'), nullable=False)
    publisher = Column(String(255), nullable=False)
    year = Column(Integer, nullable=False)
    status = Column(Enum('available', 'reserved', name='instance_status_enum'), nullable=False)
...
class Rental(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, unique=True, nullable=False)
    user_id = Column(UUID(as_uuid=True), ForeignKey('user.id'), nullable=False)
    publication_id = Column(UUID(as_uuid=True), ForeignKey('instances.id'), nullable=False)
    duration = Column(Integer, nullable=False)
    start_date = Column(String(255), nullable=False)
    end_date = Column(String(255), nullable=False)
    status = Column(Enum("active", "returned", "overdue", name="rental_status"), nullable=False)
