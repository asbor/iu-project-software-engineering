...
@router.get("/style_guidelines/{guideline_id}", response_model=schemas.StyleGuidelineBase)
async def get_style_guideline(guideline_id: int, db: Session = Depends(get_db)):
    guideline = db.query(models.StyleGuidelines).filter(
        models.StyleGuidelines.id == guideline_id).first()
    if not guideline:
        raise HTTPException(
...
async def create_style_guideline(guideline: schemas.StyleGuidelineBaseCreate, db: Session = Depends(get_db)):
    try:
...
async def delete_style_guideline(id: int, db: Session = Depends(get_db)):
    guideline = db.query(models.StyleGuidelines).filter(
        models.StyleGuidelines.id == id).first()
    if not guideline:
        raise HTTPException(
            status_code=404, detail="Style Guideline not found")
    db.delete(guideline)
...
async def update_style_guideline(id: int, guideline: schemas.StyleGuidelineBaseCreate, db: Session = Depends(get_db)):
    db_guideline = db.query(models.StyleGuidelines).filter(
        models.StyleGuidelines.id == id).first()
    if not db_guideline:
        raise HTTPException(
            status_code=404, detail="Style Guideline not found")
