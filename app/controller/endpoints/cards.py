...
def update_card(cardId: uuid.UUID, card: schemas.Card):
    ...
    if card.id is not None:
        if db_card.id != card.id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Card id must not be changed")
    
    # card magstripe must not be changed
    if card.magstripe is not None:
        if db_card.magstripe != card.magstripe:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Magstripe must not be changed")
    
    # card status must be either active, inactive or expired
    if card.status not in ["active", "inactive", "expired"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Status must be either active, inactive or expired")
    ...
    if card.created_at is not None:
        if db_card.created_at != card.created_at:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Card created_at must not be changed")
...
def create_card(card: schemas.Card):
    ...
    if db.query(models.Card).filter(models.Card.id == card.id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Card already exists")
    
    # card id must be unique
    if db.query(models.Card).filter(models.Card.id == card.id).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Card id must be unique")
    
    # card magstripe must be unique
    if db.query(models.Card).filter(models.Card.magstripe == card.magstripe).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Card magstripe must be unique")
    
    # card magstripe must be like "stringstringstringst"
    if len(card.magstripe) != 20:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Card magstripe must be like 'stringstringstringst'")
