...
def create_rental(rental: schemas.Rental):
    ...
    publication=db.query(models.Publication).filter(models.Publication.id==rental.publication_id).first()
    if publication is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Publication not found")
    
    # check if rental already exists
    rental_exists=db.query(models.Rental).filter(models.Rental.user_id==rental.user_id).filter(models.Rental.publication_id==rental.publication_id).first()
    if rental_exists is not None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Rental already exists")
...
def get_rental(rentalId: uuid.UUID):
    rental=db.query(models.Rental).filter(models.Rental.id==rentalId).first()
    return rental
...
def get_rental_by_user(userId: uuid.UUID):
    rentals=db.query(models.Rental).filter(models.Rental.user_id==userId).all()
    return rentals
...
def get_rental_by_publication(publicationId: uuid.UUID):
    rentals=db.query(models.Rental).filter(models.Rental.publication_id==publicationId).all()
    return rentals
...
def update_rental(rentalId: uuid.UUID, rental: schemas.Rental):
    rental_exists=db.query(models.Rental).filter(models.Rental.id==rentalId).first()
