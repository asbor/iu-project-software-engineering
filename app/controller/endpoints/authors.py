...
def create_author(author: schemas.Author):
    ...
    if db.query(models.Author).filter(models.Author.id == author.id).first() is not None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Author id already exists")
    
    # Length of the author name must be between 1 and 20 characters
    if len(author.name) < 1 or len(author.name) > 20:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Author name must be between 1 and 20 characters")
    
    # Length of the author surname must be between 1 and 20 characters
    if len(author.surname) < 1 or len(author.surname) > 20:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Author surname must be between 1 and 20 characters")
    ...
    if db.query(models.Author).filter(models.Author.name == author.name).filter(models.Author.surname == author.surname).first() is not None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Author name + surname already exists")
...
async def patch_author(authorId: uuid.UUID, author: schemas.Author):
    db_author = db.query(models.Author).filter(models.Author.id == authorId).first()
    ...
    if author.id is not None:
        if db_author.id != author.id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Author id cannot be changed")
    
    # The author created_at cannot be changed
    if author.created_at is not None:
        if db_author.created_at != author.created_at:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Author created_at cannot be changed")
