...
def create_reservation(reservation: schemas.Reservation):
    ...
    publication=db.query(models.Publication).filter(models.Publication.id==reservation.publication_id).first()
    if publication is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Publication not found")
    
    # check if reservation already exists
    reservation_exists=db.query(models.Reservation).filter(models.Reservation.user_id==reservation.user_id).filter(models.Reservation.publication_id==reservation.publication_id).first()
...
def get_reservation(reservationId: uuid.UUID):
    reservation=db.query(models.Reservation).filter(models.Reservation.id==reservationId).first()
    return reservation
...
def update_reservation(reservationId: uuid.UUID, reservation: schemas.Reservation):
    reservation_exists=db.query(models.Reservation).filter(models.Reservation.id==reservationId).first()
    ...
    if reservation.id != reservation_exists.id:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Reservation id can not be changed")
    
    # the created_at date can not be changed
    if reservation.created_at != reservation_exists.created_at:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Created at date can not be changed")
