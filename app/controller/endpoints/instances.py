...
def create_instance(instance: schemas.Instance):
    ...
    if instance.status != "available" and instance.status != "reserved":
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail="Status can only be changed to available or reserved")

    # check if publication exists
    publication = db.query(models.Publication).filter(
        models.Publication.id == instance.publication_id).first()
...
def get_instance(instanceId: uuid.UUID):
    instance = db.query(models.Instance).filter(
        models.Instance.id == instanceId).first()
    return instance
...
def get_instance_by_publication(publicationId: uuid.UUID):
    instances = db.query(models.Instance).filter(
        models.Instance.publication_id == publicationId).all()
    return instances
...
def get_instance_by_status(status: str):
    instances = db.query(models.Instance).filter(
        models.Instance.status == status).all()
    return instances
...
def update_instance(instanceId: uuid.UUID, instance: schemas.Instance):
    instance_exists = db.query(models.Instance).filter(
        models.Instance.id == instanceId).first()
    ...
    if instance_exists.id != instance.id:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail="Instance id can not be changed")
    ...
    if instance_exists.created_at != instance.created_at:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail=f"Created at can not be changed {instance_exists.created_at} != {instance.created_at}")

    # the updated_at fields must be updated
    if instance_exists.updated_at == instance.updated_at:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail=f"Updated at must be updated {instance_exists.updated_at} == {instance.updated_at}")
